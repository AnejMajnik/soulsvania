...
func physics_update(_delta: float) -> void:
	if slime_boss.global_position.distance_to(player.global_position) > 450 and recovered:
		switch_state.emit(chase_state)
	elif slime_boss.global_position.distance_to(player.global_position) <= 450 and recovered:
